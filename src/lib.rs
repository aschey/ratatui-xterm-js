...
use crossterm_wasm::{
    ...
    terminal::{self, init_terminal, Clear, TerminalHandle},
};
...
impl Backend for CrosstermWasmBackend {
        ...
        I: Iterator<Item = (u16, u16, &'a Cell)>,
    {
        ...
        for (x, y, cell) in content {
            ...
            if cell.fg != fg {
                let color = CColor::from(map_color(cell.fg));
                queue!(self.buffer, SetForegroundColor(color))?;
                fg = cell.fg;
            }
            if cell.bg != bg {
                let color = CColor::from(map_color(cell.bg));
    ...
    fn get_cursor(&mut self) -> io::Result<(u16, u16)> {
        crossterm_wasm::cursor::position()
    ...
    fn clear_region(&mut self, clear_type: ClearType) -> io::Result<()> {
        execute!(
            ...
            Clear(match clear_type {
                ClearType::All => crossterm_wasm::terminal::ClearType::All,
                ClearType::AfterCursor => crossterm_wasm::terminal::ClearType::FromCursorDown,
                ClearType::BeforeCursor => crossterm_wasm::terminal::ClearType::FromCursorUp,
                ClearType::CurrentLine => crossterm_wasm::terminal::ClearType::CurrentLine,
                ClearType::UntilNewLine => crossterm_wasm::terminal::ClearType::UntilNewLine,
            })
        )
    }
...
fn map_color(color: Color) -> CColor {
    match color {
